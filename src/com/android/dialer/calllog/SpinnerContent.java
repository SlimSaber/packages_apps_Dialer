...
public class SpinnerContent {
    ...
    public static List<SpinnerContent> setupSubFilterContent(Context context) {
        ...
        List<PhoneAccountHandle> accountHandles = PhoneAccountUtils.getSubscriptionPhoneAccounts(context);
        ArrayList<SpinnerContent> values = new ArrayList<SpinnerContent>(accountHandles.size() + 1);
        values.add(new SpinnerContent(CallLogQueryHandler.CALL_SIM_ALL,
                context.getString(R.string.call_log_show_all_slots)));
        for (int i = 0; i < accountHandles.size(); i++) {
            PhoneAccountHandle accountHandle = accountHandles.get(i);
            String subDisplayName = PhoneAccountUtils.getAccountLabel(context, accountHandle);
